...
class InvoiceController
{
    ...
    /**
     * GET /invoices/due
     * ?customer_id=5&search=ahmed&from=2026-01-01&to=2026-12-31&page=1&per_page=20
     */
    public static function due(Request $req): void
    {
        $filters = sanitiseInput([
            'customer_id' => $req->query('customer_id'),
        ...
        $sql = "SELECT si.id, si.quotation_id, si.customer_id, c.name AS customer_name,
                ...
                WHERE $w
                  AND (
                    si.total_amount
                    - COALESCE((SELECT SUM(amount_paid) FROM payments p WHERE p.invoice_id = si.id), 0)
                  ) > 0
        ...
        $totals = Database::fetchOne(
            "SELECT
                ...
                COALESCE(SUM((SELECT SUM(p.amount_paid) FROM payments p WHERE p.invoice_id = si.id)), 0) AS paid_amount
             FROM sales_invoices si
             JOIN customers c ON c.id = si.customer_id
             WHERE $w
               AND (
            ...
            $params
        ) ?: ['invoice_count' => 0, 'sold_amount' => 0, 'paid_amount' => 0];

        $sold = (float)$totals['sold_amount'];
        $paid = (float)$totals['paid_amount'];

        Response::success([
            'filters' => [
                'customer_id' => $filters['customer_id'] !== null ? (int)$filters['customer_id'] : null,
